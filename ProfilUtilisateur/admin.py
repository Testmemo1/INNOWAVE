...
class UtilisateurAdmin(UserAdmin):
    ...
    def profile_link_display(self, obj):
        # Prefer dynamic absolute link (SITE_BASE_URL) if available, fall back to relative URL
        abs_link = getattr(obj, 'absolute_profile_link', None)
        if abs_link:
            return format_html('<a href="{}" target="_blank">{}</a>', abs_link, abs_link)
        # fallback to relative path
        rel = obj.get_absolute_url()
        if rel:
            return format_html('<a href="{}" target="_blank">{}</a>', rel, rel)
        return "Pas de lien"
